...
const checklistItems = [
  ...
  { category: '出遊小物 🎒', items: ['常備藥品', '輕便雨衣/雨傘', '環保購物袋', '太陽眼鏡/遮陽帽', '水壺'] },
];
...
export default function Checklist() {
  return (
    <section id="checklist" className="space-y-8">
      ...
      <div className="sketch-border bg-white p-8 relative max-w-4xl mx-auto print:border-black print:border-2 print:rounded-none">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 border-b-4 border-black pb-4 gap-4">
          <div>
            <h3 className="text-3xl font-bold mb-2">✈️ 2026 日韓旅遊行李清單</h3>
        ...
        <div className="text-center mb-6 hidden print:block text-sm border-2 border-dashed border-gray-400 p-2">
          <strong>爸媽小叮嚀：</strong> 請讓小朋友自己檢查行李，有帶好的在框框內打勾 ✓ ，邊整理還可以邊把圖案塗上喜歡的顏色喔！
        </div>

        <div className="grid md:grid-cols-2 gap-x-12 gap-y-8 print:grid-cols-2 print:gap-x-12">
          {checklistItems.map((group) => (
            <div key={group.category} className="space-y-4">
              <h4 className="text-xl font-bold bg-gray-100 inline-block px-3 py-1 sketch-border print:bg-transparent print:border-2 print:border-black">{group.category}</h4>
              <ul className="space-y-3">
                {group.items.map((item) => (
                  <li key={item} className="flex items-center gap-4 font-bold text-lg">
                    <div className="w-6 h-6 border-2 border-black rounded-md shrink-0 cursor-pointer hover:bg-gray-200 transition-colors print:border-black"></div>
        ...
        <div className="mt-12 text-center hidden print:block">
           <h4 className="text-2xl font-bold mb-4">準備好出發去 🇯🇵日本 / 🇰🇷韓國 囉！</h4>
        ...
        <div className="mt-12 pt-6 border-t-2 border-dashed border-gray-300 print:hidden">
          <p className="text-center font-bold mb-4 text-lg">🛍️ 爸媽看過來！出國神隊友裝備 (買了再打勾！)</p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <motion.a 
              ...
              className="sketch-border px-6 py-3 text-center font-bold hover:bg-yellow-50 bg-white"
            >
              📱 亞洲多國上網網卡 / eSIM
            </motion.a>
            <motion.a 
